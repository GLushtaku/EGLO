...
class ApiService {
  ...
  async put<T, B = unknown>(endpoint: string, body?: B, options: RequestInit = {}): Promise<T> {
  ...
  private toAuthResponse(r: EgloApiResponse): AuthResponse {
    ...
    const email = jwtPayload["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress"];
    const name = jwtPayload["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name"];
